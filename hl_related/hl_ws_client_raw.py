...
try:
    ...
    print("pip install websockets 가 필요합니다. (pip install websockets)", file=sys.stderr)
    raise
...
class HLWSClientRaw:
        ...
        try:
            ...
            for i, st in enumerate(dex_states):
                ...
                for ap in ch.get("assetPositions") or []:
                    ...
                    if coin:
                        try:
                            norm_map[coin] = self._normalize_position(pos)
                        except Exception:
                            continue
    ...
    def build_subscriptions(self) -> List[Dict[str, Any]]:
        ...
        # 2) 포지션/자산 데이터(코인 단위) — 주소가 있을 때만
        if self.address:
            for c in self.coins:
                # for dex (ex: xyz, flx, vntl)
                if ":" in c:
                    dex = c.split(":")[0].lower()
                    coin = c.split(":")[1].upper()
                    c = f"{dex}:{coin}"
                subs.append({"type": "activeAssetData", "user": self.address, "coin": c})
            # 3) 유저 스냅샷(멀티 DEX 종합 상태)
            ...
            if isinstance(data, dict) and isinstance(data.get("mids"), dict):
                ...
                for raw_key, raw_mid in mids.items():
                    ...
                    if isinstance(raw_key, str) and raw_key.startswith("@"):
                        ...
                        # 1-1) 페어 가격 캐시
                        old_pair_px = self.spot_pair_prices.get(pair_name)
                        self.spot_pair_prices[pair_name] = px
                        self._spot_log_pair(
                            pair_name, logging.INFO,
                            f"@{pair_idx} -> {pair_name} (asset_id={asset_id}) {old_pair_px} -> {px}"
                        )

                        # 1-2) 쿼트가 USDC인 경우 base 단일 가격도 채움
                        if quote == "USDC":
                            old_base_px = self.spot_prices.get(base)
                            self.spot_prices[base] = px
                            self._spot_log_pair(
                                pair_name, logging.DEBUG,
                                f"base({base}) px {old_base_px} -> {px}"
                            )
                    ...
                    if maybe_spot_base:
                        ...
                        if px is not None:
                            ...
                            oldp = self.spot_pair_prices.get(pair_name)
                            self.spot_pair_prices[pair_name] = px
                            self._spot_log_pair(pair_name, logging.DEBUG, f"text-pair px {oldp} -> {px}")

                            if pair_name.endswith("/USDC"):
                                oldb = self.spot_prices.get(maybe_spot_base)
                                self.spot_prices[maybe_spot_base] = px
                                self._spot_log_pair(pair_name, logging.DEBUG,
                                    f"base({maybe_spot_base}) px {oldb} -> {px}")
                        n_pair_text += 1
                        continue
